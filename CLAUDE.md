...
**Browser-console warnings from `contentscript.js` are NOT ours.** Lines like `MaxListenersExceededWarning: Possible EventEmitter memory leak ... close listeners` and `ObjectMultiplex - orphaned data for stream "app-init-liveness"` originate in a browser **extension's** injected content script (these are MetaMask/crypto-wallet stream-multiplexing signatures), not this app — the frontend is plain HTML + Alpine.js with no Node `EventEmitter` or stream plumbing. Don't chase them; confirm by reloading in an incognito window with extensions disabled (the warnings disappear).
...
- **Folder-listing path is ambiguous (singular vs plural).** The published reference and various clients disagree on whether it's `/folder/{id}/files` or `/folders/{id}/files`; the singular form was observed returning **404 "Resource not found" (`404.0400`)** against a real account even though the same `rootFolderId` is accepted as a `folderId` by `POST /upload`. **Live validation confirmed the *plural* form `GET /v1_2/folders/{id}/files` is the one that works (200); the singular 404s.** Because the spelling isn't reliably documented (and could differ by account/region), `listFolderFiles` **probes the candidates** (`folderFilesPaths = {"/folders/%s/files", "/folder/%s/files"}`, plural first) and uses the first that doesn't 404, logging `4shared folder listing path resolved` with the winner — so the happy path costs one request. A broken listing endpoint silently disables conflict detection (`resolveConflicts` logs the lookup failure as a warn and treats it as "no conflict"), so a duplicate name then surfaces only as a `403.0201` at upload time — see the next bullet. The listing response wraps the array as `{"files":[...]}`; `decodeFileList` also accepts a bare array.
